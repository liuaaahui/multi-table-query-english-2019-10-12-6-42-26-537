...
select * from student where id in(select studentId from student_course where courseId = 1 or courseId = 2 group by studentId having count(score)='2');
...
select AVG(score) FROM student_course GROUP BY studentId ORDER BY AVG(score) DESC,courseId ASC;
